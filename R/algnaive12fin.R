algnaive12=function(data,cc,copred,thetaini,y.col=3,coords.col=1:2,covar=F,covar.col,fix.nugget=T,nugget,kappa=0,cov.model="exponential",trend){
  ...
  if(nrow(data)!=nrow(as.matrix(cc))) stop ("data and cc does not have the same number of lines")
...
gneiting, gneiting.matern, pure.nugget')
  ...
  if((length(y) == 0) | (length(cc) == 0)|(length(coords) == 0)|(length(copred) == 0)){
  ...
  out=naive12(data=data,cc=cc,covar=covar,covar.col=covar.col,copred=copred,thetaini=thetaini,y.col=y.col,coords.col=coords.col,fix.nugget=fix.nugget,nugget=nugget,kappa=kappa,trend=trend)
...
mu = X*beta"
  ...
  #out=naive12(data=data,cc=cc,covar=covar,covar.col=covar.col,copred=copred,thetaini=thetaini,y.col=y.col,coords.col=coords.col,fix.nugget=fix.nugget,nugget=nugget,kappa=kappa,cutof=cutof,trend=trend)
