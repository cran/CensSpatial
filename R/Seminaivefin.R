Seminaive=function(data,y.col,coords.col,covar,covar.col,copred,cov.model="exponential",thetaini,fix.nugget=T,nugget,kappa=0,cons,MaxIter,cc,trend){
...
gneiting, gneiting.matern, pure.nugget')
  ...
  if(covar==F){
    if(class(trend)=="formula") stop("trend must be cte,1st or 2nd when covar=F")
  }


  out=suppressWarnings(seminaive(data=data,y.col=y.col,covar=covar,coords.col=coords.col,covar.col=covar.col,cov.model=cov.model,thetaini=thetaini,fix.nugget=fix.nugget,nugget=nugget,kappa=kappa,cons=cons,MaxIter=MaxIter,cc=cc,copred=copred,trend=trend))
